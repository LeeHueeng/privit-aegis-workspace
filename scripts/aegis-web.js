  ...
  <script>
    ...
    const detectionGuideItems = [
      ...
      },
      {
        ...
        detects: {
          ko: "ID-bearing route, API version/legacy route, OpenAPI security scheme/operation security posture, mass-assignment 민감 필드, GraphQL endpoint/IDE/schema 신호, OIDC/OAuth discovery metadata 품질, JWKS key 품질, OAuth/SSO callback, authorization request parameter, WebAuthn related-origin 메타데이터, 사용자/세션 API 익명 노출, API 캐시/nosniff 신호를 인벤토리합니다.",
          en: "Inventories ID-bearing routes, API version/legacy routes, OpenAPI security scheme/operation security posture, mass-assignment sensitive fields, GraphQL endpoint/IDE/schema signals, OIDC/OAuth discovery metadata quality, JWKS key quality, OAuth/SSO callbacks, authorization request parameters, WebAuthn related-origin metadata, anonymous user/session API exposure, and API cache/nosniff signals.",
          ja: "ID含有route、API version/legacy route、OpenAPI security scheme/operation security posture、mass-assignment系の機密field、GraphQL endpoint/IDE/schema信号、OIDC/OAuth discovery metadata品質、JWKS key品質、OAuth/SSO callback、authorization request parameter、WebAuthn related-originメタデータ、ユーザー/セッションAPIの匿名露出、API cache/nosniff信号をインベントリします。",
          zh: "盘点带 ID 的路由、API version/legacy route、OpenAPI security scheme/operation security posture、mass-assignment 敏感字段、GraphQL endpoint/IDE/schema 信号、OIDC/OAuth discovery metadata 质量、JWKS key 质量、OAuth/SSO callback、authorization request parameter、WebAuthn related-origin 元数据、用户/会话 API 匿名暴露以及 API cache/nosniff 信号。"
        },
        criteria: {
          ko: "자동 passive 검사는 후보를 식별하고, legacy/beta/internal API route와 GraphQL 공개 IDE/schema 신호, OpenAPI cleartext server URL, missing security scheme/requirement, anonymous operation, query API key, mass-assignment 필드, cleartext discovery endpoint, none alg, JWKS duplicate kid/private key material을 검토 대상으로 표시해야 합니다.",
          en: "Passive checks identify candidates and mark legacy/beta/internal API routes, public GraphQL IDE/schema signals, OpenAPI cleartext server URLs, missing security schemes/requirements, anonymous operations, query API keys, mass-assignment fields, cleartext discovery endpoints, none alg, and JWKS duplicate kid/private key material for review.",
          ja: "passive検査は候補を識別し、legacy/beta/internal API route、公開GraphQL IDE/schema信号、OpenAPI cleartext server URL、missing security scheme/requirement、anonymous operation、query API key、mass-assignment field、cleartext discovery endpoint、none alg、JWKS duplicate kid/private key materialをレビュー対象として示します。",
          zh: "Passive 检查识别候选项，并标记 legacy/beta/internal API route、公开 GraphQL IDE/schema 信号、OpenAPI cleartext server URL、missing security scheme/requirement、anonymous operation、query API key、mass-assignment 字段、cleartext discovery endpoint、none alg 和 JWKS duplicate kid/private key material 供复核。"
        },
        evidence: {
          ko: "path, API version label, OpenAPI security scheme type/server host/operation count, parameter/field 이름, method, 상태, discovery endpoint host/protocol/path, JWKS key count/kty/use/alg/kid 상태, callback cache/referrer header, GraphQL/identity/user API/cache/header signal을 기록하고 OAuth parameter 값은 저장하지 않습니다.",
          en: "Records path, API version label, OpenAPI security scheme type/server host/operation count, parameter/field names, method, status, discovery endpoint host/protocol/path, JWKS key count/kty/use/alg/kid state, callback cache/referrer headers, and GraphQL/identity/user API/cache/header signals without storing OAuth parameter values.",
          ja: "path、API version label、OpenAPI security scheme type/server host/operation count、parameter/field名、method、status、discovery endpoint host/protocol/path、JWKS key count/kty/use/alg/kid状態、callback cache/referrer header、GraphQL/identity/user API/cache/header signalを記録し、OAuth parameter値は保存しません。",
          zh: "记录 path、API version label、OpenAPI security scheme type/server host/operation count、parameter/field 名称、method、状态、discovery endpoint host/protocol/path、JWKS key count/kty/use/alg/kid 状态、callback cache/referrer header 以及 GraphQL/identity/user API/cache/header 信号，不保存 OAuth parameter 值。"
        }
      },
      {
        icon: "10",
