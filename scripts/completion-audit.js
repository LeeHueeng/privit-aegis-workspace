...
function main() {
  ...
  add(
    ...
    "Target advisory includes passive probes plus DNS/CNAME takeover fingerprints, Host header reflection, reverse tabnabbing, SRI, mixed content, CORS, CSP quality, CSP Report-Only inventory, COOP/COEP/CORP isolation header posture, Permissions-Policy quality and deprecated Feature-Policy checks, cookie scope, dynamic route cache-deception candidate inventory, logout cache/Clear-Site-Data cleanup signals, OAuth/SSO callback cache/referrer posture, OAuth authorization request hardening inventory, WebAuthn/passkey route inventory, WebAuthn related-origin metadata checks, MFA/2FA/OTP/security-setting route inventory and cache posture, registration/signup route and rate-limit header inventory, framework fingerprinting, redirect/HPP parameter inventory, external redirect destination inventory, SSRF-style URL/webhook/proxy input inventory, mass-assignment sensitive field inventory, auth form GET checks, CSRF token candidate inventory, external/cleartext form action checks, sensitive URL query/fragment parameter checks, auth-flow token URL inventory, account-recovery route inventory, visible auth rate-limit header inventory, auth API cache/nosniff checks, security.txt metadata inventory, mobile App/Universal Link association inventory, file upload form inventory, unauthenticated user/session API checks, client bundle leakage, DOM XSS, Web Messaging, resource manipulation, client template injection, prototype pollution, browser storage, WebSocket, JWT, XSSI JSON, cloud storage references, backup/extension exposure, directory listings, source maps, API docs, API version/legacy route inventory, GraphQL endpoints and schema/IDE exposure signals, upload/import/export surfaces, identity metadata, attack-surface matrix, admin/debug surfaces, metafiles, error disclosure, HTTP methods, header misconfiguration, and TLS posture.",
