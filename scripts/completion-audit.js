...
function main() {
  ...
  add(
    ...
    "Target advisory includes passive probes plus DNS/CNAME takeover fingerprints, Host header reflection, reverse tabnabbing, SRI, mixed content, CORS, CSP quality, CSP Report-Only inventory, COOP/COEP/CORP isolation header posture, cookie scope, logout cache/Clear-Site-Data cleanup signals, OAuth/SSO callback cache/referrer posture, OAuth authorization request hardening inventory, framework fingerprinting, redirect/HPP parameter inventory, auth form GET checks, CSRF token candidate inventory, external/cleartext form action checks, sensitive URL query/fragment parameter checks, auth-flow token URL inventory, account-recovery route inventory, visible auth rate-limit header inventory, auth API cache/nosniff checks, security.txt metadata inventory, file upload form inventory, unauthenticated user/session API checks, client bundle leakage, DOM XSS, Web Messaging, resource manipulation, client template injection, prototype pollution, browser storage, WebSocket, JWT, XSSI JSON, cloud storage references, backup/extension exposure, directory listings, source maps, API docs, GraphQL endpoints, upload/import/export surfaces, identity metadata, attack-surface matrix, admin/debug surfaces, metafiles, error disclosure, HTTP methods, header misconfiguration, and TLS posture.",
