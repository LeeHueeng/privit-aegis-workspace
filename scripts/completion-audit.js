...
function main() {
  ...
  add(
    ...
    "Target advisory includes passive probes plus DNS/CNAME takeover fingerprints, Host header reflection, reverse tabnabbing, CORS, CSP quality, cookie scope, framework fingerprinting, redirect/HPP parameter inventory, auth form GET checks, CSRF token candidate inventory, external/cleartext form action checks, sensitive URL parameter checks, file upload form inventory, client bundle leakage, DOM XSS, Web Messaging, resource manipulation, client template injection, prototype pollution, browser storage, WebSocket, JWT, XSSI JSON, cloud storage references, backup/extension exposure, directory listings, source maps, API docs, GraphQL endpoints, upload/import/export surfaces, identity metadata, attack-surface matrix, admin/debug surfaces, metafiles, error disclosure, HTTP methods, header misconfiguration, and TLS posture.",
